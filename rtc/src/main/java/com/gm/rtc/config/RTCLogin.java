...
import com.gm.rtc.model.RTCHandler;
...
public class RTCLogin {
	...
	public RTCHandler veridyRTC(String repositoryUri, String username, String password, String workspace, String component) {
		RTCHandler rtcHandler = new RTCHandler(); 
		TeamPlatform.startup();
        try {            
            IProgressMonitor monitor = new SysoutProgressMonitor();
            ITeamRepository repo = login(repositoryUri, username, password, monitor);
            rtcHandler.setRepo(repo);
            IWorkspaceConnection conn = findWorkspace(repo, workspace, monitor);
//            if (conn == null) {
//                return;
//            }
            rtcHandler.setConn(conn);
            IComponentHandle cmp = findComponentInWorkspace(conn, component, monitor);
//            if (cmp == null) {
//                return;
//            }
            rtcHandler.setCmp(cmp);
    ...
    public static void main(String[] args) {
        if (args.length != 5) {
            System.err.println("Usage: " + RTCLogin.class.getName() + " <repository uri> <username> <password> <workspace> <component>");
            return;
        }
        TeamPlatform.startup();
        try {            
